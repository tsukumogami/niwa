...
// This file carries the steps behind the dual-agent acceptance scenarios in
// features/codex-agent.feature, which take PRD-dual-agent-workspace's
// acceptance criteria end to end through the compiled binary.
//
// Two conventions run through all of them, both from the PRD:
//
...
//     and no model. The rules it mirrors are DESIGN-dual-agent-workspace's
//     measured findings 1 and 2: a bounded walk from the nearest project-root
//     marker down to the working directory, one file per directory by strict
//     precedence.
...
const (
	// codexPayloadDir is the payload directory niwa owns at the instance root
	// and links into every repository working tree.
	...
	// A chain larger than this is exactly the case the declared budget exists
...
var projectDocMaxBytesRE = regexp.MustCompile(`(?m)^\s*project_doc_max_bytes\s*=\s*(\d+)`)

func registerCodexDualAgentSteps(ctx *godog.ScenarioContext) {
	// Fixture staging.
	...
	ctx.Step(`^the Codex context at "([^"]*)" selects nothing$`, theCodexContextAtSelectsNothing)
	...
	ctx.Step(`^the Codex context at "([^"]*)" fits the budget declared by instance "([^"]*)"$`, theCodexContextAtFitsDeclaredBudget)

	// Payload, links, and skills.
	ctx.Step(`^the Codex payload of instance "([^"]*)" is reachable from "([^"]*)"$`, theCodexPayloadIsReachableFrom)
	ctx.Step(`^the Codex payload of instance "([^"]*)" has exactly (\d+) skills links?$`, theCodexPayloadHasNSkillsLinks)
	ctx.Step(`^the Codex payload of instance "([^"]*)" has exactly (\d+) config files?$`, theCodexPayloadHasNConfigFiles)
	ctx.Step(`^the Codex skills link "([^"]*)" of instance "([^"]*)" mirrors "([^"]*)"$`, theCodexSkillsLinkMirrors)
	ctx.Step(`^instance "([^"]*)" declares no Codex hooks$`, theInstanceDeclaresNoCodexHooks)
	ctx.Step(`^the Codex payload of instance "([^"]*)" declares no credentials$`, theCodexPayloadDeclaresNoCredentials)
	...
	ctx.Step(`^the git exclude of repo "([^"]*)" in instance "([^"]*)" carries the bare Codex patterns$`, theGitExcludeCarriesBareCodexPatterns)
...
func aStagedFileOfSizeEndingWith(ctx context.Context, path string, size int, sentinel string) (context.Context, error) {
	...
	const filler = "Filler line that pads this layer toward the declared budget.\n"
...
// directory (or a committed `.codex/` directory) has to commit something in it.
...
// tree with the staged files plus a new workspace.toml and force-pushes it.
// It is how the refresh criterion changes the workspace's configured content
// between two applies.
...
//
// This is the offline proxy the PRD's acceptance criteria are decided against.
// Its discriminating power is the selection itself: a test that only asserted
...
func theCodexContextAtSelectsNothing(ctx context.Context, loc string) error {
	s := getState(ctx)
	if s == nil {
		return fmt.Errorf("no test state")
	}
	...
	if len(chain) != 0 {
		return fmt.Errorf("Codex context at %s selects %v, want nothing", loc, chain)
	}
	return nil
}
...
// oversized: without the budget niwa declares, a session standing here would
// read a chain larger than Codex's 32768-byte default and lose its tail in
// silence.
func theCodexContextAtExceedsDefaultBudget(ctx context.Context, loc string) error {
	...
	size, err := codexChainBytes(chain)
	if err != nil {
		return err
	}
	if size <= codexDefaultBudget {
		return fmt.Errorf("Codex context at %s is %d bytes, which the %d-byte default already covers; the fixture is not exercising the budget", loc, size, codexDefaultBudget)
	}
	return nil
}

func theCodexContextAtFitsDeclaredBudget(ctx context.Context, loc, instance string) error {
	s := getState(ctx)
	if s == nil {
		return fmt.Errorf("no test state")
	}
	...
	budget, err := declaredCodexBudget(s, instance)
	if err != nil {
		return err
	}
	if budget < size {
		return fmt.Errorf("instance %q declares project_doc_max_bytes = %d, below the %d-byte chain at %s: the innermost layer would be cut in silence", instance, budget, size, loc)
	}
	return nil
}

func codexChainBytes(chain []string) (int, error) {
	total := 0
	for _, path := range chain {
...
// declaredCodexBudget reads project_doc_max_bytes out of the payload config.
func declaredCodexBudget(s *testState, instance string) (int, error) {
	path := filepath.Join(s.workspaceRoot, instance, codexPayloadDir, "config.toml")
	data, err := os.ReadFile(path)
	if err != nil {
		return 0, fmt.Errorf("reading %s: %w", path, err)
	}
	m := projectDocMaxBytesRE.FindSubmatch(data)
	if m == nil {
		return 0, fmt.Errorf("%s declares no project_doc_max_bytes:\n%s", path, data)
	}
	var budget int
	if _, err := fmt.Sscanf(string(m[1]), "%d", &budget); err != nil {
		return 0, fmt.Errorf("parsing project_doc_max_bytes from %s: %w", path, err)
	}
	return budget, nil
}

// --- payload, links, skills -------------------------------------------

// theCodexPayloadIsReachableFrom reads the payload config through the working
// tree's own `.codex` entry, which is the path a session standing there takes.
// Asserting on the instance-root payload instead would pass with the link
// missing, dangling, or pointed somewhere else entirely.
func theCodexPayloadIsReachableFrom(ctx context.Context, instance, loc string) error {
	s := getState(ctx)
	if s == nil {
		return fmt.Errorf("no test state")
	}
	...
	viaLink := filepath.Join(dir, codexPayloadDir, "config.toml")
	linked, err := os.ReadFile(viaLink)
	if err != nil {
		return fmt.Errorf("reading the payload config through %s: %w", filepath.Join(dir, codexPayloadDir), err)
	}
	direct, err := os.ReadFile(filepath.Join(s.workspaceRoot, instance, codexPayloadDir, "config.toml"))
	if err != nil {
		return fmt.Errorf("reading the instance payload config: %w", err)
	}
	if !bytes.Equal(linked, direct) {
		return fmt.Errorf("the payload reached from %s differs from instance %q's payload", loc, instance)
	}
	return nil
}

func theCodexPayloadHasNSkillsLinks(ctx context.Context, instance string, want int) error {
	s := getState(ctx)
	if s == nil {
		return fmt.Errorf("no test state")
	}
	skillsDir := filepath.Join(s.workspaceRoot, instance, codexPayloadDir, "skills")
	...
	if len(names) != want {
		return fmt.Errorf("instance %q payload holds %d skills entries %v, want %d", instance, len(names), names, want)
	}
	return nil
}

func theCodexPayloadHasNConfigFiles(ctx context.Context, instance string, want int) error {
	s := getState(ctx)
	if s == nil {
		return fmt.Errorf("no test state")
	}
	payload := filepath.Join(s.workspaceRoot, instance, codexPayloadDir)
	...
	if count != want {
		return fmt.Errorf("instance %q payload holds %d .toml files, want %d", instance, count, want)
	}
	return nil
}

// theCodexSkillsLinkMirrors asserts the delivered plugin root is the source
// tree: same file set, byte-identical content, nothing added or omitted.
func theCodexSkillsLinkMirrors(ctx context.Context, plugin, instance, sourceLoc string) error {
	s := getState(ctx)
	if s == nil {
		return fmt.Errorf("no test state")
	}
	link := filepath.Join(s.workspaceRoot, instance, codexPayloadDir, "skills", plugin)
	// Resolving the link is half the assertion: a dangling one fails here, and
	// the walk below needs a real directory to descend into.
	delivered, err := filepath.EvalSymlinks(link)
	if err != nil {
...
// tree here is inside a clone, and `.git` is not part of the plugin.
...
// instance payload and the developer's Codex home -- for hook definitions or
// hook state. The scan deliberately does not follow the skills symlinks: a
// plugin may ship its own Claude-side hooks, and those are the plugin's, not
// something niwa declared.
func theInstanceDeclaresNoCodexHooks(ctx context.Context, instance string) error {
	...
	roots := []string{
		filepath.Join(s.workspaceRoot, instance, codexPayloadDir),
		filepath.Join(s.homeDir, ".codex"),
	}
...
// theCodexPayloadDeclaresNoCredentials asserts the payload carries no API key
...
func theCodexPayloadDeclaresNoCredentials(ctx context.Context, instance string) error {
	s := getState(ctx)
	if s == nil {
		return fmt.Errorf("no test state")
	}
...
// theGitExcludeCarriesBareCodexPatterns pins the exact pattern text. A
// trailing slash would make each pattern directory-only, and git classifies a
// symlink as a file, so `.codex/` would leave `?? .codex` in git status
// forever -- the feature's highest-risk detail, and invisible to a substring
// check that a slashed pattern would still satisfy.
func theGitExcludeCarriesBareCodexPatterns(ctx context.Context, repo, instance string) error {
	s := getState(ctx)
	if s == nil {
		return fmt.Errorf("no test state")
	}
	...
	for _, want := range []string{codexPayloadDir, codexOverrideName} {
		...
		case 1:
		case 0:
			return fmt.Errorf("%s carries no bare %q pattern:\n%s", path, want, data)
		default:
