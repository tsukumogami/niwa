...
- [ ] Success message: a stderr WARNING-style block matching the prominence of the `--rebind` precedent at `init.go:351-359`. Includes the workspace path, the `niwa-bootstrap` branch name, and next steps: `git show HEAD` to inspect, `git push -u origin niwa-bootstrap` to publish, then `niwa apply`.
