...
type GlobalSettings struct {
	...
	// WatchSandbox is the single `niwa watch --once` containment switch. The
	// dispatched review session always runs under the developer's real HOME,
	// environment, and Claude daemon; this switch only decides whether the OS
	// no-egress sandbox is applied. "required" (the default) applies the OS
	// no-egress sandbox and refuses to dispatch when it cannot be enforced on this
