...
// undeclaredProviderCfg builds a config with one declared named provider
// ("known") and a secret referencing an undeclared provider ("unknown"),
// so resolution hits the undeclared-provider branch of resolveOne. The
// req string is appended to the vault URI (e.g. "?required=false").
func undeclaredProviderCfg(req string) *config.WorkspaceConfig {
...
// TestResolveWorkspaceUndeclaredProviderErrorsByDefault: a reference to a
// provider not declared in the active bundle hard-fails (wrapping
// ErrKeyNotFound) when neither ?required=false nor AllowMissing applies.
func TestResolveWorkspaceUndeclaredProviderErrorsByDefault(t *testing.T) {
	_, err := resolve.ResolveWorkspace(context.Background(), undeclaredProviderCfg(""), resolve.ResolveOptions{
		Registry: newFakeRegistry(t),
	})
	if err == nil {
		t.Fatal("expected error for undeclared provider")
	}
	if !errors.Is(err, vault.ErrKeyNotFound) {
		t.Errorf("expected ErrKeyNotFound, got %v", err)
	}
	if !strings.Contains(err.Error(), "not declared") {
		t.Errorf("expected message to mention 'not declared', got %q", err.Error())
	}
}

// TestResolveWorkspaceUndeclaredProviderOptionalDowngrades: a ?required=false
// reference to an undeclared provider downgrades silently to empty, matching
// the missing-key contract. This is the graceful-degradation the worktree
// overlay-vault path depends on.
func TestResolveWorkspaceUndeclaredProviderOptionalDowngrades(t *testing.T) {
	var stderr bytes.Buffer
	out, err := resolve.ResolveWorkspace(context.Background(), undeclaredProviderCfg("?required=false"), resolve.ResolveOptions{
		Registry: newFakeRegistry(t),
		Stderr:   &stderr,
	})
	if err != nil {
	...
	if out.Env.Secrets.Values["S"].IsSecret() {
		t.Error("optional undeclared-provider ref should downgrade to empty, not secret")
	}
	if stderr.Len() != 0 {
		t.Errorf("optional downgrade must not log, got %q", stderr.String())
	}
}

// TestResolveWorkspaceUndeclaredProviderAllowMissingDowngrades: with
// AllowMissing, an undeclared-provider reference downgrades to empty with a
// stderr warning instead of hard-failing the resolution.
func TestResolveWorkspaceUndeclaredProviderAllowMissingDowngrades(t *testing.T) {
	var stderr bytes.Buffer
	out, err := resolve.ResolveWorkspace(context.Background(), undeclaredProviderCfg(""), resolve.ResolveOptions{
		Registry:     newFakeRegistry(t),
		AllowMissing: true,
		Stderr:       &stderr,
	})
	...
	if out.Env.Secrets.Values["S"].IsSecret() {
		t.Error("expected downgrade to empty under AllowMissing")
	}
	if !strings.Contains(stderr.String(), "--allow-missing-secrets") {
		t.Errorf("expected warning to reference the flag, got %q", stderr.String())
	}
	if !strings.Contains(stderr.String(), "not declared") {
		t.Errorf("expected warning to mention the undeclared provider, got %q", stderr.String())
	}
}
