...
func TestPick_EnterAtFirstRow(t *testing.T) {
	choices := []Choice{
		{Name: "alpha", Description: "first instance"},
		{Name: "beta", Description: "second instance"},
	}
	stdin := bytes.NewReader([]byte{'\r'}) // Enter
	var stderr bytes.Buffer

	idx, err := pick(stdin, &stderr, "Pick an instance:", choices)
	if err != nil {
	...
	out := stderr.String()
	if !strings.Contains(out, "Pick an instance:") {
...
func TestPick_DescriptionWithAnsiEscapeIsStripped(t *testing.T) {
	...
	// clearing). What we want to confirm is that the caller-sourced
