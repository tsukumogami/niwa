...
// by correlating the jobs-dir state.json whose cwd equals instanceDir (D3).
// Because instanceDir is a freshly-created unique path, cwd == instanceDir is
// an exact disambiguation key, stronger than the probabilistic short id.
//
...
// UUID (workspace.ValidSessionID), it returns that id. A state.json that
// matches the cwd but carries an invalid/empty sessionId is treated as
// not-yet-ready and polling continues until timeout.
//
...
// nil/zero.
func captureSessionID(jobsDir, instanceDir string, timeout time.Duration, now func() time.Time, poll time.Duration) (string, error) {
	if now == nil {
		now = time.Now
	}
	if poll <= 0 {
	...
	for {
		id, ambiguous, err := matchSessionByCwd(jobsDir, targetDir)
		if err != nil {
			return "", err
		}
		if ambiguous {
...
// returning the validated sessionId of the unique job whose normalized cwd
// equals targetDir. It reports ambiguous=true when more than one state.json
// claims targetDir. A match with an invalid/empty sessionId is ignored (not
// counted as a match), so the caller keeps polling until a valid id appears.
func matchSessionByCwd(jobsDir, targetDir string) (id string, ambiguous bool, err error) {
	...
	for _, e := range entries {
		...
		// is not yet ready (the worker may not have recorded a UUID yet).
