...
func TestApplyPlanDeliverTreeErrorsLoudly(t *testing.T) {
	dir := t.TempDir()
	target := filepath.Join(dir, "skills")

	entry := newPlanEntry(agentplan.OpDeliverTree, target, "")
	entry.Source = dir

	_, _, err := applyPlan(&agentplan.Plan{Entries: []agentplan.Entry{entry}})
	if !errors.Is(err, errNoExecutorArm) {
		t.Fatalf("err = %v, want errNoExecutorArm", err)
	}
	if _, statErr := os.Stat(target); !os.IsNotExist(statErr) {
		t.Errorf("unimplemented op touched %s (stat err %v)", target, statErr)
	}
}
