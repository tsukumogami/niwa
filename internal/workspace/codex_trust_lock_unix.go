...
const (
	...
	// finish its read-modify-write. Generous against a slow disk, short enough
	// that a stuck holder surfaces as an error rather than an apply that never
	// returns.
	codexTrustLockTimeout = 30 * time.Second

	// codexTrustLockPoll is the interval between acquisition attempts. The lock
	// is taken non-blocking and retried so the deadline above is real: a
	// blocking flock has no timeout to give.
