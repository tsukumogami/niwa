...
func TestApplyThresholdError(t *testing.T) {
	tmpDir := t.TempDir()
	niwaDir := filepath.Join(tmpDir, ".niwa")
...
`
	...
	// Return 11 repos to exceed default threshold of 10.
	repos := make([]github.Repo, 11)
	for i := range repos {
		repos[i] = github.Repo{Name: "repo", Visibility: "public"}
	}
	...
	applier := NewApplier(mockClient)
	err = applier.Apply(context.Background(), cfg, niwaDir, filepath.Join(tmpDir, "instance"))
	if err == nil {
		t.Fatal("expected threshold error, got nil")
	}
	if !strings.Contains(err.Error(), "threshold") {
		t.Errorf("error = %q, want threshold-related message", err.Error())
	}
}
