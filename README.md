...
| `niwa dispatch "<task>" [--name <slug>] [--detach]` | Launch a background Claude Code worker for `<task>` in its own ephemeral instance. Set `remote_control_on_dispatch = true` in `~/.config/niwa/config.toml` `[global]` to start dispatched workers with Claude Code Remote on, so you can steer them from Agent View / mobile -- see `docs/guides/remote-control-on-dispatch.md` |
