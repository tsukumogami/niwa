...
- `docs/guides/codex-agent.md` — running Codex in a prepared workspace: choosing which agent niwa launches (`--launch-agent`, `NIWA_AGENT`, and the two `default_agent` settings), background dispatch and its costs, what a Codex session gets, the generated gap list of what it doesn't, and what niwa writes into the developer's own Codex configuration
