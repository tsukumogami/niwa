...
- `docs/guides/dual-agent-workspace.md` — every instance is prepared for both Claude Code and Codex, so the agent choice is never made at creation time: the instance payload and byte budget, per-repo `.codex` link and composed `AGENTS.override.md`, trust entries, conflicts with committed content, worktree parity, deliberate exclusions
